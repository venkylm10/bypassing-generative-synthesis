...
branch for one or more missing modalities.
...
`missing` may be `None` (all 4 real encoders), a single modality name (one
missing branch), or a list/tuple/set of modality names (multiple missing
branches — this ablation drops both t1c and t2w simultaneously). Every
missing branch at a given scale is imputed with the SAME NMA value: the
average over whatever modalities remain available at that scale.
"""
import torch
...
def _missing_set(missing):
    """Normalize `missing` (None / str / iterable of str) to a set of names."""
    if missing is None:
        return set()
...
class FusionUNet3D(nn.Module):
    ...
    (missing modalities' tensors are ignored/may be absent) and `missing` is
    the modality name(s) to impute via NMA (None to use all 4 real encoders,
    e.g. for the GAN-imputed / zero-fill comparison pipelines; a single name
    or a list/set of names for one or more missing branches).
    """
    ...
    def forward(self, volumes, missing=None):
        ...
        for s in range(n_scales):
            scale_feats = [per_modal_feats[m][s] for m in MODALITIES if m not in missing_set]
            if missing_set:
                imputed = normalized_mean_impute(scale_feats)
                # every missing branch gets the SAME imputed value; reinsert in
                # canonical modality order for a fixed fusion-conv input layout
                ordered = [imputed if m in missing_set else per_modal_feats[m][s] for m in MODALITIES]
            else:
