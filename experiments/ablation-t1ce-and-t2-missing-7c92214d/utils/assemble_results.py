...
def main():
    ...
    per_class_names = ["background", "necrotic_core", "edema", "enhancing_tumor"]
    ...
    results = [
        {
            ...
            "method": "latent_imputed_unet_t1ce_t2_missing",
            "description": ("Mean Dice score (foreground classes: necrotic core, edema, "
                             "enhancing tumor; background excluded) of the latent-imputed "
                             "3D U-Net on held-out test subjects when BOTH T1ce (t1c) and "
                             "T2 (t2w) are simultaneously missing (Normalized Mean Algorithm "
                             "aggregates latents from the remaining t1n and t2f encoders at "
                             "every scale), averaged across "
                             f"{n_seeds} random seeds. Higher is better; 1.0 = perfect overlap. "
                             "Reported as an absolute robustness figure per this ablation's "
                             "objective (severe dual-modality dropout); no non-inferiority "
                             "claim against a GAN/zero-fill dual-missing baseline is made here, "
                             "since those were not rerun for this dual-missing scenario."),
        ...
        },
        {
            ...
            "method": "latent_imputed_unet_t1ce_t2_missing",
            "description": "Mean Dice score for the necrotic/non-enhancing core class (raw BraTS labels 1 and 3 merged), T1ce+T2-missing scenario, averaged across seeds.",
        ...
        },
        {
            ...
            "method": "latent_imputed_unet_t1ce_t2_missing",
            "description": "Mean Dice score for the peritumoral edema class (raw BraTS label 2), T1ce+T2-missing scenario, averaged across seeds.",
        ...
        },
        {
            ...
            "method": "latent_imputed_unet_t1ce_t2_missing",
            "description": "Mean Dice score for the enhancing tumor class (raw BraTS label 4), T1ce+T2-missing scenario, averaged across seeds.",
        ...
        },
        {
            ...
            "description": ("End-to-end forward-pass latency of the latent-imputed architecture "
                             "(2 real modality encoders [t1n, t2f] + 2 NMA-imputed branches "
                             "[t1c, t2w] + shared decoder) for a single 96^3 volume on the eval "
                             "GPU, averaged across seeds (30 timed reps after 5 warmup iterations each)."),
        ...
        },
        {
            ...
            "method": "latent_imputed_unet_t1ce_t2_missing",
            "description": "Peak CUDA memory allocated during a single-subject inference forward pass, averaged across seeds.",
    ...
    baselines = []  # baseline_umm_gan / baseline_zero_fill artifacts not present in this pod at run time — see report.md

    figures = []

    # --- Figure 1: dice score distribution across validation/test subjects ---
    fig1_inline = {"per_subject_dice": all_subject_dice, "mean_dice_score": mean_dice_score}
    fig1_path = os.path.join(FIGURES_DIR, "double_missing_dice_distribution.png")
    ...
    plt.ylabel("Count")
    plt.title(f"Dice distribution under T1ce+T2 dual dropout "
              f"({len(all_subject_dice)} test-subject evals, {n_seeds} seeds)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(fig1_path, dpi=150)
    plt.close()
    figures.append({
        "name": "double_missing_dice_distribution",
    ...
    plt.title("Training curve (seed 0, T1ce+T2 missing)")
    plt.tight_layout()
        ...
        "config": {
            **cfg,
            ...
            "architecture": "FusionUNet3D: 4 independent per-modality encoders, "
                             ...
                             "(here, 2 of the 4 encoder branches [t1c, t2w] are unused/"
                             "NMA-imputed since both are dropped simultaneously)",
            "missing_modality": cfg["missing_modality"],
            "imputation_method": "Normalized Mean Algorithm (deterministic, no training) "
                                  "applied per-scale to the remaining available modalities' "
                                  "(t1n, t2f) encoder feature maps; the same NMA value is "
                                  "reinserted into both missing branches (t1c, t2w)",
        },
        "results": results,
        "baselines": baselines,
        "figures": figures,
